...
export const _js = (i) => {
    ...
    return new Promise(resolve => resolve(eval(i.options.getString("code"))))
        .then(result => typeof result === "string" ? result : inspect(result,{ showHidden: true }))
...
export const Embedlize = (i,type) => 
    (type === "js" ? _js(i) : type === "sh" ? _sh(i) : new Promise(resolve => resolve()))
        ...
        .then(str => str.length > 4086 ? {embeds:[new Embed().setAuthor({name:"実行結果",iconURL:"attachment://ok.png"}).setDescription("ファイルに出力されました！")],files:[new MessageAttachment(Buffer.from(str,"utf-8"),`result.${type}`),new MessageAttachment("./assets/ok.png","ok.png")]}: {embeds:[new Embed().setAuthor({name:"実行結果",iconURL:"attachment://ok.png"}).setDescription(`\`\`\`${type}\n${str}\n\`\`\``)],files:[new MessageAttachment("./assets/ok.png","ok.png")]})