...
export const exec = async function(i){
    ...
    if(player.player.pause())return i.reply({embeds:[new Embed().setAuthor({name: "情報", iconURL: "attachment://info.png"}).setDescription("一時停止しました")],files: [new MessageAttachment("./assets/info.png","info.png")]})
    return i.reply({embeds:[new Embed().setAuthor({name: "エラー", iconURL: "attachment://error.png"}).setDescription("既に一時停止しています")],files:[new MessageAttachment("./assets/error.png","error.png")]})
}